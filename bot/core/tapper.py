...
class Tapper:
    ...
    def __init__(self, tg_client: Client, lock: asyncio.Lock, account_data: dict):
    ...
    async def get_auth_url(self) -> str:
        if self.proxy:
            proxy = Proxy.from_str(self.proxy) if self.proxy else None
            proxy_dict = dict(
    ...
    async def login(self, http_client: aiohttp.ClientSession, auth_url: str, proxy: str) -> tuple[dict[str], str]:
        ...
        try:
            async with self.lock:
                response_text, x_cv, x_touch = login_in_browser(auth_url, proxy=self.proxy)
    ...
    async def run(self) -> None:
        ...
        # Используем self.proxy вместо proxy
        proxy_conn = ProxyConnector().from_url(self.proxy) if self.proxy else None

        http_client = CloudflareScraper(headers=headers, connector=proxy_conn)

        if self.proxy:
            await self.check_proxy(http_client=http_client, proxy=Proxy.from_str(self.proxy))

        # Используем self.get_auth_url() вместо get_auth_url(proxy)
        ...
        while True:
            try:
                if http_client.closed:
                    ...
                    # Используем custom_headers с user-agent
                    custom_headers = headers.copy()
                    if self.user_agent:
                        custom_headers['User-Agent'] = self.user_agent
                    
                    http_client = CloudflareScraper(headers=custom_headers, connector=proxy_conn)

            # ... остальной код метода run остается без изменений ...
...
async def run_tapper(tg_client: Client, account_data: dict, lock: asyncio.Lock):
    try:
        await Tapper(
            tg_client=tg_client,
            lock=lock,
