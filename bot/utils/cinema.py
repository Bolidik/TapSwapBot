...
def load_answers_from_file() -> dict:
    ...
    with open(MISSIONS_FILENAME, 'r', encoding='utf-8') as csvfile:
...
def save_missions_to_file(missions):
    ...
    with open(MISSIONS_FILENAME, 'w', encoding='utf-8', newline='') as f:
...
def get_visible_cinema_missions(account_data: dict) -> list[Mission]:
    ...
    return sorted(actual, key=lambda m: m.start_at if m.start_at else datetime.now(), reverse=True)[:settings.VISIBLE_CINEMA_MISSIONS]
